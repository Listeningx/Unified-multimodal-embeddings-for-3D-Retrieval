...
from models.losses_multimodal import Uni3dMultimodalLoss, get_multimodal_loss
import collections
...
class MultimodalTrainingWrapper(nn.Module):
    ...
    def forward(self, pc, image, text):
        """
        前向传播
        ...
        Returns:
            dict with:
                - 'fused_embed': [B, 30, trans_dim] 原始融合特征序列
                - 'fused_projected': [B, clip_embed_dim] 投影后的融合特征（用于对比学习）
                - 'clip_text_embed': [B, clip_embed_dim] CLIP 文本特征
                - 'clip_image_embed': [B, clip_embed_dim] CLIP 图像特征
                - 'logit_scale': 温度参数
        """
        # 1. 使用 Uni3D 编码融合特征 (ivt)
        if self.use_embed:
            # 使用预提取特征
            uni3d_output = self.uni3d.forward(
                pc=pc, 
                image_embed=image,  # 预提取的图像特征
                text_embed=text,    # 预提取的文本特征
                modal='ivt'
            )
        else:
            # 实时编码
        ...
        # 这些是未经模型投影的原始预提取特征
        clip_text_embed = uni3d_output['txt_feats']  # [B, clip_embed_dim]
        clip_image_embed = uni3d_output['image_feats']  # [B, clip_embed_dim]

        return {
            'fused_feats': fused_feats,           # 原始融合特征序列（可用于其他任务）
...
def train_one_epoch(model, dataloader, clip_model, optimizer, scheduler, scaler, criterion, 
    ...
    """训练一个 epoch（参考 main.py 的 train 函数，支持 DeepSpeed 和梯度累积）
    ...
    for data_iter, inputs in enumerate(dataloader):
        ...
        if args.enable_deepspeed:
            ...
            with amp.autocast(enabled=not args.disable_amp):
                # 前向传播
                outputs = model(pc=feature, image=image, text=texts)
                
                # 计算损失（DeepSpeed 自动处理累积，不需要手动除以 accum_steps）
                loss_dict = criterion(outputs)
                loss = loss_dict['loss']

            if not math.isfinite(loss.item()):
                nan_count += 1
                logging.warning(f"⚠️ NaN/Inf loss detected at step {data_iter}, batch {data_iter}. Loss={loss.item()}. NaN count: {nan_count}/{max_nan_tolerance}")
