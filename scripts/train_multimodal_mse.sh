...
# 
# 与 train_multimodal_dropout.sh 的区别：
# - 使用 train_multimodal_mse.py 而不是 train_multimodal_dropout.py
# - 在前几轮添加 MSE 损失，让 fused_emb 和图文特征之间对齐
# - MSE 权重从初始值逐渐降低到 0
#
# MSE 对齐损失配置：
# - mse_weight: 初始 MSE 权重（默认 1.0）
# - mse_warmup_epochs: 保持完整权重的 epoch 数（默认 2）
# - mse_decay_epochs: 衰减到 0 所需的 epoch 数（默认 3）
...
# 使用方法: 
#   bash scripts/train_multimodal_mse.sh          # 默认使用多卡 DDP
#   bash scripts/train_multimodal_mse.sh single   # 使用单机单卡
#   bash scripts/train_multimodal_mse.sh multi    # 使用多卡 DDP
#   bash scripts/train_multimodal_mse.sh deepspeed # 使用 DeepSpeed
TRAIN_MODE=${1:-"multi"}
...
# enable_mse_loss: 是否启用 MSE 对齐损失（默认启用）
# 在训练初期帮助 fused_emb 快速对齐到 CLIP 图文特征空间
enable_mse_loss=true

# mse_weight: MSE 损失的初始权重
mse_weight=1.0

# mse_text_weight / mse_image_weight: MSE 文本/图像对齐损失的权重
mse_text_weight=1.0
mse_image_weight=1.0

# mse_warmup_epochs: MSE 损失保持完整权重的 epoch 数
# 建议设置为 2，让模型在前两轮充分对齐
mse_warmup_epochs=2

# mse_decay_epochs: MSE 损失权重衰减到 0 所需的 epoch 数
# 建议设置为 3，让权重逐渐过渡到 0
mse_decay_epochs=3

# mse_decay_type: MSE 损失权重衰减方式
# - linear: 线性衰减（默认）
# - cosine: 余弦衰减（平滑过渡）
# - exponential: 指数衰减（快速下降）
mse_decay_type="linear"
...
modal_prob_ivt=0.50   # 完整三模态
modal_prob_iv=0.15    # 图像+点云
modal_prob_vt=0.15    # 点云+文本
modal_prob_v=0.10     # 仅点云（核心模态）
...
RESUME_DIR="/apdcephfs/share_303565425/DCC3/serenasnliu/GAR/Uni3D/output_multimodal/20260129_181548/checkpoints"  # 例如：./output_multimodal/20260128_175627
...
run_single_gpu() {
    echo "===========================================" 
    echo "Starting Multimodal Uni3D Training (Single GPU + MSE Alignment)"
    echo "===========================================" 
    ...
    echo "Enable MSE Alignment: ${enable_mse_loss}"
    echo "MSE Weight: ${mse_weight}, Warmup: ${mse_warmup_epochs}, Decay: ${mse_decay_epochs}"
    echo "==========================================="
    ...
    # MSE 对齐损失参数
    if [ "$enable_mse_loss" = "true" ]; then
        cmd_args="$cmd_args --enable_mse_loss"
        cmd_args="$cmd_args --mse_weight $mse_weight"
        cmd_args="$cmd_args --mse_text_weight $mse_text_weight"
        cmd_args="$cmd_args --mse_image_weight $mse_image_weight"
        cmd_args="$cmd_args --mse_warmup_epochs $mse_warmup_epochs"
        cmd_args="$cmd_args --mse_decay_epochs $mse_decay_epochs"
        cmd_args="$cmd_args --mse_decay_type $mse_decay_type"
    else
        cmd_args="$cmd_args --no_mse_loss"
    fi

    python train_multimodal_mse.py \
        --model $model \
...
case "$TRAIN_MODE" in
    ...
    *)
        ...
        echo "Usage: bash scripts/train_multimodal_mse.sh [single|multi|deepspeed]"
        exit 1
        ;;
esac
